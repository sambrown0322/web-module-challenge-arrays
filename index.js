...
function getAverageWordLength(array) {
  let total = 0;
  for (let i = 0; i < array.length; i++) {
    total = total + array[i].split(" ").length;
  }
  total = Math.round(total / array.length);
  return total;
}
...
let item = originalFlavors[Math.floor(Math.random() * originalFlavors.length)];
console.log(item);
